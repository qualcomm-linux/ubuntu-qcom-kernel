...
#
# Gated on ESRT_CONFIRMED=1: this branch's meaning is "firmware reported
# success but the content belongs to another kernel" (suspected_dtb_rollback).
# When firmware instead reported failure (ESRT_CONFIRMED=0), the same
# ROLLBACK_TARGET_KVER match means something different —
# apply_failed_with_rollback_available, handled below — and must not be
# shadowed by this branch's unconditional exit.
